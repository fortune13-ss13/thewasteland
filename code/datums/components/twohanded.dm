...
/datum/component/two_handed/proc/wield(mob/living/carbon/user)
	...
	if(ismonkey(user))
		...
		return
	var/obj/item/other_item = user.get_inactive_held_item() // Not sure if this is faster than just doing two get_inactives
	if(other_item)
		if(require_twohands)
			to_chat(user, "<span class='notice'>[parent] is too cumbersome to carry in one hand!</span>")
			user.dropItemToGround(parent, force=TRUE)
		else
			user.dropItemToGround(other_item, force=FALSE) //Drop the item in our other hand if possible, for fast wielding
			if(user.get_inactive_held_item()) // If you cannot remove the item in your hand, don't try and wield.
